...
export function stringsToIntegers(numbers: string[]): number[] {
    const converted = numbers.map((str: string): number => parseInt(str) || 0);
    return converted;
}
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const shoutMessages = messages.map((message: string): string =>
        message.endsWith("!") ? message.toUpperCase() : message
    );
    const newMessages = shoutMessages.filter(
...
export function injectPositive(values: number[]): number[] {
    if (values.find((value: number): boolean => value < 0)) {
        ...
        const prevArray = newArray.slice(0, negative);
        const sum = prevArray.reduce(
            (total: number, num: number) => total + num,
            0
        );
        newArray.splice(negative + 1, 0, sum);
        return newArray;
    } else {
